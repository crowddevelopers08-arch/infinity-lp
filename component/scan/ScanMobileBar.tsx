...
import { LuPhone, LuScanFace } from "react-icons/lu"
import { track } from "../track"
import { PHONE_TEL } from "./scanData"

export function ScanMobileBar() {
  return (
    <aside
    ...
    >
      <div className="mx-auto grid w-full max-w-xl grid-cols-2 gap-2.5">
        ...
        <a
          href="#scan"
          className="btn-wave relative flex min-h-12 items-center justify-center gap-2 overflow-hidden rounded-full bg-[#f52227] px-4 py-3 text-sm font-bold text-white no-underline shadow-[0_8px_22px_rgba(245,34,39,0.26)] transition-all duration-200 hover:bg-[#cf1c20] active:scale-[0.98]"
        >
          <LuScanFace className="relative z-10 size-[18px]" strokeWidth={2.3} aria-hidden="true" />
          <span className="relative z-10">Scan My Scalp</span>
        </a>
      </div>
    </aside>
  )
